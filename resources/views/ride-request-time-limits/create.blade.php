@extends('layouts.app')
@php
	$currentPage = 'ride-request-time-limits';
@endphp
@section('title', __('Create Ride Request Time Limit'))
@section('content')
<div class="container">
	<h1>{{ __('Create Ride Request Time Limit') }}</h1>
	<div class="mb-3">
		<a href="{{ route('ride-request-time-limits.index') }}" class="btn btn-secondary btn-sm me-1"> <i class="fa fa-arrow-right"></i> {{__('Back to')}} {{__('Ride Request Time Limits')}}</a>
	</div>
	<div class="main-card mb-3 card">
		<div class="card-body">
			<form method='POST' action='{{ route('ride-request-time-limits.store') }}' class="needs-validation" novalidate>
				@csrf
				<x-form-input 
					name="time_limit_seconds"
					type="text"
					label="{{__('Time Limit Seconds')}}"
					required
				/>
				<button type='submit' class="btn btn-primary btn-sm me-1">{{ __('Add') }}</button>
			</form>
		</div>
	</div>
</div>@endsection