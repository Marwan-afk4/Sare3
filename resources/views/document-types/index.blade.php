...
<div class="container-fluid">
	...
	<div class="mb-3">
		<a wire:navigate href="{{ route('document-types.create') }}" class="btn btn-primary btn-sm me-1">{{__('Create Document Type')}} <i class="fa fa-plus"></i></a>
	</div>
	<div class='main-card mb-3 card'>
		<div class='card-body'>
			<table class="mb-0 table table-hover">
				<tr>
					<th>
						<a wire:navigate href="{{ route('document-types.index', ['sort' => 'id', 'order' => $sortOrder === 'asc' ? 'desc' : 'asc']) }}">
							{{ __("Id") }}
							@if($sortField === 'id')<i class="text-danger">{{ $sortOrder === 'asc' ? '▼' : '▲' }}</i>@endif
						</a>
					</th>
					<th>
						<a wire:navigate href="{{ route('document-types.index', ['sort' => 'name', 'order' => $sortOrder === 'asc' ? 'desc' : 'asc']) }}">
							{{ __("Name") }}
							@if($sortField === 'name')<i class="text-danger">{{ $sortOrder === 'asc' ? '▼' : '▲' }}</i>@endif
						</a>
					</th>
					<th>
						<a wire:navigate href="{{ route('document-types.index', ['sort' => 'is_required', 'order' => $sortOrder === 'asc' ? 'desc' : 'asc']) }}">
							{{ __("Is Required") }}
							@if($sortField === 'is_required')<i class="text-danger">{{ $sortOrder === 'asc' ? '▼' : '▲' }}</i>@endif
						</a>
					</th>
					<th>
						<a wire:navigate href="{{ route('document-types.index', ['sort' => 'created_at', 'order' => $sortOrder === 'asc' ? 'desc' : 'asc']) }}">
							{{ __("Created At") }}
				...
				<tr>
					...
					<td class="text-center">
						<a wire:navigate href='{{ route('document-types.show', $documentType) }}' class="btn btn-subtle-primary btn-sm me-1">{{ __("Details") }} <i class="fa fa-eye"></i></a>
