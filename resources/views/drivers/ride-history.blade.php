...
@php
    $currentPage = 'drivers';
@endphp
@section('title', $driver->name . ' - Ride History')
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1>{{ $driver->name }} - {{ __('Ride History') }}</h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('drivers.index') }}">{{ __('Drivers') }}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('drivers.show', $driver) }}">{{ $driver->name }}</a></li>
                        <li class="breadcrumb-item active">{{ __('Ride History') }}</li>
                    </ol>
                </nav>
            </div>
            <div>
                <a href="{{ route('drivers.show', $driver) }}" class="btn btn-secondary btn-sm">
            ...
            <div class="card-body">
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-3 d-flex align-items-end">
                        ...
                        <a href="{{ route('drivers.ride-history', $driver) }}" class="btn btn-outline-secondary ms-2">{{ __('Reset') }}</a>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                @else
                    <div class="text-center py-5">
                        ...
                        <p class="text-muted">{{ __('This driver has not completed any rides yet.') }}</p>
                    </div>
                @endif
            </div>
        </div>
