...
class DaroryControllers extends Command
{
    protected $signature = 'darory:controllers {model?} {--all} {--missing}';
    protected $description = 'Generate Laravel controllers based on existing models';
    ...
    public function handle()
    {
        ...
        $this->info("✨ Darory Controllers Generator ✨");
        ...
        foreach ($models as $index => $model) {
            $modelData[] = [
                '#'          => $index + 1,
                'Model'      => $model,
                'Controller' => $this->fileStatus("Http/Controllers/{$model}Controller"),
        ...
        if ($this->option('missing')) {
            ...
            $missingModels = array_filter($models, function ($model) {
                return !File::exists(app_path("Http/Controllers/{$model}Controller.php"));
            });

            if (empty($missingModels)) {
                $this->info("✅ All controllers already exist.");
                return;
            }
        ...
        if ($selectedIndexes === 'm') {
            $this->call('darory:controllers', ['--missing' => true]);
            return;
        }
    ...
    protected function generateController($modelName)
    {
        ...
        $useStatements = $this->generateUseStatements($modelName, $relations);
        ...
        $template = str_replace(
            ['{{useStatements}}', '{{model}}' ,'{{tableName}}', '{{modelVariable}}', '{{modelsVariable}}', '{{withQuery}}', '{{relationsList}}', '{{createReturnStatement}}', '{{editReturnStatement}}','{{kebabName}}'],
            [
                $useStatements,
                $modelName,
                $tableName,
                ...
                $withQuery,
                $relationsList,
                "return view('{$kebabName}.create'" . ($compactVariables ? ", compact($compactVariables)" : "") . ");",
                "return view('{$kebabName}.edit', compact($editCompact));",
                $kebabName
            ],
            $template
        );
    ...
    protected function generateUseStatements($modelName, $relations)
    {
