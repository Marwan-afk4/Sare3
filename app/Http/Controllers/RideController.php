...
class RideController extends Controller
{
    ...
    public function abnormal(Request $request)
    {
        ...
        // Paginated results based on active tab — ordered by ride id desc
        // (newest ride first) on both tabs.
        if ($activeTab === 'suspicious') {
            $rides = $suspiciousQuery->orderBy('id', 'desc')->paginate(30)->appends($request->query());
        } else {
            $rides = $multiCaptainQuery->orderBy('id', 'desc')->paginate(30)->appends($request->query());
        }
